...
{
	/// <summary>
	/// BHM (Bashicu Hydra Matrix) 引擎
	/// 基于BMS的扩展，增加了待定坏根、预展开式、字典序比较等概念
	/// </summary>
	public class BhmEngine
	{
		...
		private int FindBadRoot(int[][] matrix)
		{
			...
			// 获取基准式 (最右侧待定坏根的预展开式)
			int[][] baseExpansion = GetPreExpansion(matrix, candidates.Last());
			...
			for (int i = candidates.Count - 1; i >= 0; i--)
			{
				...
				// 检查是否为小根 (规则12)
				if (IsSmallRoot(preExpansion, baseExpansion))
				{
					badRoot = candidate;
					break;
				}
		...
		private int[] ComputeDelta(int[][] matrix, int badRoot, int rows)
		{
			...
			{
				// 如果第 i+1 行元素为零，阶差向量的第 i 行也为零
				if (i + 1 < rows && matrix[i + 1][cols - 1] == 0)
				{
					delta[i] = 0;
				}
				else
		...
		/// 带详细信息的展开
		/// </summary>
		public BhmExpansionResult ExpandWithDetails(int[][] matrix, int n)
		{
		...
		{
			return "BHM (Bashicu Hydra Matrix)";
		}
	}
	...
	/// BHM 展开结果
	/// </summary>
	public class BhmExpansionResult
	{
